//! Price queries against the DuckDB `prices_today` table.
//!
//! The `prices_today` table is expected to have been loaded (from `AllPricesToday.json.gz`,
//! flattened into NDJSON and imported into DuckDB) at SDK initialization time, before
//! creating a `PriceQuery`.
...
/// Query interface for MTG card prices backed by the `prices_today` DuckDB table.
...
impl<'a> PriceQuery<'a> {
    ...
    ///
    /// Returns a nested object keyed by `source -> provider -> category -> finish -> date -> price`.
    pub fn get(&self, uuid: &str) -> Result<Value> {
        let (sql, params) = SqlBuilder::new("prices_today")
            .where_eq("uuid", uuid)
            .order_by(&["date DESC"])
            .build();
        ...
        // Build a nested map: source -> provider -> currency -> category -> finish -> {date: price}
    ...
    /// Get the most recent price for each provider/category/finish group for a card UUID.
    pub fn today(&self, uuid: &str) -> Result<Vec<Value>> {
        let sql = r#"
            SELECT *
            FROM prices_today
            WHERE uuid = ?
              AND date = (SELECT MAX(date) FROM prices_today WHERE uuid = ?)
        "#;
    ...
    pub fn history(
        ...
        let mut qb = SqlBuilder::new("prices_today");
    ...
    ///
    /// Joins `cards` to `prices_today` and returns the printing with the lowest price.
